...
Перед запуском необходимо установить хост в переменной окружения DB_URL - localhost
...
1. ### **В заголовке `Authorization` необходимо передавать токен (любой string) для эндпоинтов, требующих авторизации**. 
Так как в openapi.yml для некоторых ручек необходима авторизация - для проверки токенов создан **middleware-заглушка**, который можно переписать для проверки ролей пользователей. 
...
1. ✅ **Эндпоинт статистики** — добавлена метрика `pr_lifecycle_duration_hours` в Prometheus, которая отслеживает время жизни PR от создания до мерджа в часах. Метрика доступна на эндпоинте `/metrics`, а работать с ней можно через интерфейс Prometheus по адресу `http://localhost:9090`.
...
**Что сделал:** Этот флаг у PR по умолчанию `false`, становится `true` когда не хватает ревьюеров при `reassignedReviewers` или `createPullRequest`. Это позволяет клиенту понять, что PR создан, но для него не хватило доступных ревьюверов.
